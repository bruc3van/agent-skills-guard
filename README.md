...
- ⚙️ **可配置策略**：Default / Strict 两种内置预设，按需切换扫描严格程度
...
1. **策略加载** — 加载 ScanPolicy（Default / Strict 两种预设）
...
- **多格式支持** — 扫描所有文本文件，自动跳过二进制/图片/字体/编译产物等惰性文件；单条规则可通过 `file_types` 限定特定扩展名（如 `.py`、`.js`、`.ps1`），通用规则（密钥泄露、破坏性命令等）对所有文本文件生效
...
1. **规则放置**：通用安全基线放 `core_rules.yaml`，Cisco 对齐放 `cisco_parity_signatures.yaml`
2. **hard_trigger**：只有明确危险、安装时不能放行的规则才设为 `true`
3. **file_types**：仅匹配特定文件类型时使用
4. **exclude_patterns**：高噪声规则必须补充排除模式
5. **fixture**：新增规则须提供正例，调整现有规则须补充负例

### 文档路径降级机制

扫描器对位于文档路径（`docs/`、`examples/`、`test/`、`references/` 等）中的文件实行降级处理：

- **skip_in_docs**：策略中配置的规则在文档路径中直接跳过（如 `CURL_POST`、`PY_EVAL`）
- **非 hard_trigger 规则**：在文档路径中自动降低严重度和权重
- **PROMPT_INJECTION_ 规则**：即使是 hard_trigger，在文档路径中也会降级（但不跳过）
- **真实高危规则**：hard_trigger 规则（密钥泄露、破坏性命令等）在文档路径中**不降级**，保持阻断

**SKILL.md 特殊处理**：`SKILL.md` 作为 skill 主文件，不受文档路径降级影响，始终按完整规则执行扫描。

**策略差异**：
- `default`：平衡模式，合理的文档示例不会阻断安装
- `strict`：更严格，减少文档降级范围，增加结构校验和归档深度扫描
