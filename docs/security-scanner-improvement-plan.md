...
| 缺少 file magic 检测 | 无法发现 `.py/.md/.json` 等扩展名伪装成可执行文件/归档文件 |
...
- 保留现有 `CURL_PIPE_SH` 与 `CURL_PIPE_SH_MENTION` 抑制逻辑：同一行实际执行规则命中时，不再重复输出“文本/字符串提及”类 finding。
...
- 明确 `ThreatCategory` 到现有 `IssueCategory` 的映射：prompt/social/policy 可映射为 `Other` 或新增可选扩展字段；command/code execution 映射 `DangerousFunction` / `ProcessExecution`；data exfiltration/secrets/sensitive file 映射 `DataExfiltration` / `FileSystem`；resource abuse/obfuscation/supply chain 映射 `ObfuscatedCode` / `Other`。若前端可扩展，优先新增 `threat_category` 可选字段保留 Cisco 分类语义。
...
- `SKILL.md` 全规则扫描和 `CURL_PIPE_SH`/`CURL_PIPE_SH_MENTION` 同行抑制逻辑保留。
...
- manifest compatibility 与代码行为一致性检查
- description 质量/关键词诱导检查
...
- 引用文件提取覆盖 Markdown 链接、自然语言引用指令、执行型引用、`@reference:`、`include/import/load:`、Python import 和 assets/references/templates 路径，并能区分 stdlib/第三方与本地模块。
- Read/Write/Bash/Grep/Glob/Network allowed-tools 分项能按实际行为告警，Python helper 脚本存在本身不告警。
- strict structure 白名单校验覆盖允许子目录、允许扩展名、UTF-8、NUL/binary、frontmatter、name/description/compatibility。
...
3. P2：Archive extraction、File magic、Pipeline/Compound 分析。
4. P3：Analyzability、Finding 归一化、报告 metadata、分类映射。
