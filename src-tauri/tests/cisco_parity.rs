//! Cisco parity integration tests (manifest-driven fixtures)
...
fn scan_options_for_case(case: &ParityCase) -> ScanOptions {
...
#[test]
fn parity_manifest_case_count() {
    let manifest = load_manifest();
    assert!(
        manifest.cases.len() >= 60,
        "expected at least 60 parity cases, got {}",
        manifest.cases.len()
    );
}

#[test]
fn parity_strict_policy_has_lower_file_limit() {
...
#[test]
fn parity_report_metadata_has_policy_fingerprint() {
...
#[test]
fn parity_severity_override_demotes_issue() {
    ...
    policy.severity_overrides.push(SeverityOverride {
        ...
        reason: "test: demote for parity".to_string(),
    });
