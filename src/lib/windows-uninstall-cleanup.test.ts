...
const cleanupFragmentPath = path.join(repoRoot, "src-tauri", "wix", "cleanup-app-data.wxs");
...
describe("Windows MSI uninstall cleanup", () => {
  it("wires an MSI fragment that removes only application-owned data directories", () => {
    ...
    expect(tauriConfig.bundle.windows.wix.fragmentPaths).toContain("wix/cleanup-app-data.wxs");
